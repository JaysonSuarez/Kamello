...
import { ArrowLeft, Zap, Star, CreditCard } from "lucide-react";
...
export default function Pricing() {
  ...
  const packs = [
    { 
      name: t('pricing_pack_basic'), 
      price: "15.000", 
      ops: "5 OPS", 
      description: t('pricing_pack_basic_desc'), 
      icon: <Zap className="w-6 h-6 text-[#ff7665]" /> 
    },
    { 
      name: t('pricing_pack_pro'), 
      price: "35.000", 
      ops: "15 OPS", 
      description: t('pricing_pack_pro_desc'), 
      ...
      <main className="mx-auto max-w-[1100px] px-6 py-12">
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-1.5 rounded-full bg-[#ff7665]/10 text-[#ff7665] text-sm font-bold tracking-wider uppercase mb-4">
            {t('pricing_transparent')}
          </span>
          <h1 className="font-serif text-5xl md:text-6xl mb-6">
            {t('pricing_hero_1')}
            <span className="text-[#ff7665]">{t('pricing_hero_2')}</span>
          </h1>
          <p className="text-[#5f6a79] text-xl max-w-2xl mx-auto">{t('marketing_description')}</p>
        </div>
        ...
        <section className="mb-24">
          <div className="flex items-center gap-3 mb-10">
            ...
            <h2 className="text-2xl font-extrabold tracking-tight">{t('pricing_ops_label')}</h2>
          </div>
            ...
            {packs.map((pack, i) => (
              <div key={i} className={`flex flex-col p-8 rounded-[32px] bg-white border-2 ${pack.popular ? 'border-[#ff7665] shadow-xl' : 'border-[#efe7e2]'} transition-transform hover:scale-[1.01]`}>
                <div className="flex justify-between items-start mb-6">
                  ...
                  {pack.popular && <span className="bg-[#ff7665] text-white px-4 py-1 rounded-full text-xs font-bold">{t('pricing_recommended')}</span>}
                </div>
                ...
                <div className="flex items-center gap-2 text-[#00cba9] font-bold mb-6">
                  <Zap className="w-4 h-4" /> {pack.ops}
                </div>
        ...
        <section className="mt-24 p-12 rounded-[48px] bg-[#1f2c45] text-center text-white relative overflow-hidden">
          ...
          <h2 className="text-3xl font-serif mb-6 relative z-10">{t('pricing_why_title')}</h2>
